...
audioGlobRef.value = audioRef.value}
    if (track != null) { 
  marqueText = `Трек: ${track.title || ' --- '} | Артист: ${track.artist || ' --- '} | Альбом: ${activeTrack1.value.album || ' --- '} | Year: ${ activeTrack1.value.year || ' -- '} | Поздравительный комент: ${activeTrack1.value.happy || ' --- '} | Длительность: ${Math.floor(activeTrack1.value.duration/60)+' min '+Math.round(activeTrack1.value.duration%60) || track.duration } сек`;
