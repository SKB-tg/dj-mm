...
<template>
  ...
  <div class="audio-player" v-if="!props.stop1"><label for="audio-player">Audio-player #1</label>
    <!-- Текущий трек -->
    <!-- Текущий трек -->
    ...
    <div class="controls-player" >
       ...
       <button @click="togglePlayPause" class="btn btn-primary">
        ...
        <span v-if="isPlaying"><img v-if="isPlaying" src="/pause.svg" width="18" height="19"  style="top: 3px;margin: 0;position: relative; color: aliceblue;"/></span>
        </button>
...
.audio-player {
  ...
  margin: 8px;
  max-width: 300px;
  box-shadow:  0px 3px 3px 5px rgba(0, 255, 255, 0.7); 
}
.morquee-player {
  margin-top: 5px;}
.track-info {
...
.btn-secondary-left { background: #278268; color: white;
  border-radius:  50%; /*2px 16px 16px 2px;*/
...
.btn-primary { background: #278268; color: white;
 ...
 .btn-secondary-right { background: #278268; color: white;
  border-radius: 50%; /* 16px 16px 16px 16px;*/
    width: 40px;
}
 .progress {
  width: 100%;
